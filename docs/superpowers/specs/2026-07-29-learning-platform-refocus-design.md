...
У діалозі рекомендацій джерела ще не мають `id`, тому діалог присвоює їх один раз при отриманні відповіді (`useMemo` над `resources`), і далі працює з `LearningResource`, а не з DTO. Це дозволяє `ResourceList` мати один тип на вході.
